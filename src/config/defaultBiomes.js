/**
 * Default biome definitions (Whittaker-style)
 *
 * Extracted from whittaker.js for use as the built-in fallback
 * when callers don't provide custom biome configuration.
 */

/** Default biome definitions with ID, name, display color, and climate ranges */
...
const BIOME_COLOR_MAP = new Map(DEFAULT_BIOMES.map(b => [b.id, b.color]));

/** Pre-built name lookup */
const BIOME_NAME_MAP = new Map(DEFAULT_BIOMES.map(b => [b.id, b.name]));

/** Biomes checked via temp/humidity ranges (evaluation order = priority) */
const GRID_BIOMES = [
  DEFAULT_BIOMES[2],  // desert  — hot + dry
  DEFAULT_BIOMES[5],  // jungle  — hot + wet
  DEFAULT_BIOMES[6],  // swamp   — moderate + very wet
  DEFAULT_BIOMES[4],  // forest  — moderate + wet
  DEFAULT_BIOMES[7],  // tundra  — cold + dry-to-moderate
  DEFAULT_BIOMES[3],  // grassland — fallback moderate
];

/**
 * Default Whittaker-style biome classifier.
 *
 ...
 */
export function defaultClassify(temp, humidity, elevation, seaLevel) {
...
/**
 * Get RGB color for a biome ID (default palette)
...
/**
 * Get biome name for a biome ID (default palette)
 * @param {number} biomeId
 * @returns {string}
 */
